...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  table.sources td { padding: 8px 10px 8px 0; border-bottom: 1px solid var(--line);
                     vertical-align: top; }
  .notice.integrity code { font-family: ui-monospace, Consolas, monospace;
                           font-size: 11.5px; word-break: break-all; color: var(--fg); }
...
<div class="notice integrity">
  <strong>Report integrity.</strong>
  The findings in this report hash to
  <code>{{.Digest}}</code>
  (SHA-256, computed over finding content only, excluding timestamps).
  Quote this reference separately from the document — in the engagement record
  or covering email — so a later copy can be checked against what was actually
  produced. It is a reference, not a signature: anyone editing the findings can
  recompute it.
  <br><br>
  Assessed against <strong>{{.CatalogueVer}}</strong>. Control text is
  reproduced from the framework as published; where a control could not be
  cleanly extracted from the source document it is marked for verification in
  the catalogue rather than presented as authoritative.
</div>
